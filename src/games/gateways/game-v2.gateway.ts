...
interface IGameSocketMessage {
  // generate a qr code using this room string
  // and display it on the cashier's screen to be scanned by the player
  ...
  handleJoinRoom(client: Socket, message: IGameSocketMessage) {
    ...
    if (message.gameData.gameState === GameStateEnum.CREATED) {
      ...
      if (message.soundLang === 'am') {
        ...
        soundUrl = `http://localhost:3000/api/games/balls-audio/am/${
        ...
        soundUrl = `http://localhost:3000/api/games/balls-audio/or/${
          message.gameData.playingNumbers[message.gameData.currentIndex]
        }or.m4a`;
      }
